<style>
    .red-icon {
        color: red;
    }
</style>

<table class="table table-bordered" id="example" cellspacing="0" width="100%">
    <thead>
        <tr>
            <th style="width:5%; text-align:center;">No.</th>
            <th style="width:15%; text-align:center;">Nama</th>
            <th style="width:10%; text-align:center;">Satuan</th>
            <th style="width:15%; text-align:center;">Jumlah</th>
            <th style="text-align:center;">Aksi</th>
        </tr>
    </thead>
    <tbody>
        @foreach ($sampahIndustris as $index => $sampahIndustri)
        <tr>
            <td style="width:5%">{{ $index + 1 }}</td>
            <td style="width:15%">{{ $sampahIndustri->nama }}</td>
            <td style="width:10%; text-align:center;">{{ $sampahIndustri->satuan }}</td>
            <td style="width:15%">Rp. {{ number_format($sampahIndustri->jumlah) }}</td>
            <td style="width:15%; text-align:center;">
                <form style="display: inline;" onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('sampahIndustri.destroy', $sampahIndustri->id) }}" method="POST">
                    @csrf
                    @method('DELETE')
                    <button type="submit" style="background: none; border: none; padding: 0; margin: 0; cursor: pointer;">
                        <i class="align-middle red-icon" data-feather="trash"></i>
                    </button>
                </form>
            </td>
        </tr>
        @endforeach
    </tbody>
</table>